...
const (
	...
	GetAppointmentQuery                  = "SELECT appointment.id id, appointment.case_type case_type, appointment.is_vip is_vip, appointment.start_at start_at, appointment.user_id user_id, appointment.info info, appointment.income income, appointment.status appointment_status,  appointment.updated_at updated_at, user.fname user_fname, user.lname user_lname, user.id user_id, user.gender user_gender from appointment LEFT JOIN user on appointment.user_id = user.id WHERE id = ?"
	...
	GetOrganizationHolidaysQuery         = "SELECT holiday.id id, holiday.title, holiday.hdate, holiday.organization_id, organization.name organization_name FROM holiday LEFT JOIN organization ON holiday.organization_id = organization.id WHERE holiday.organization_id = ? AND DATE(holiday.hdate) between ? AND ? ORDER BY hdate ASC"
	GetAdminHolidaysQuery                = "SELECT holiday.id id, holiday.title, holiday.hdate, holiday.organization_id, organization.name organization_name FROM holiday LEFT JOIN organization ON holiday.organization_id = organization.id LIMIT 10 OFFSET ?"
