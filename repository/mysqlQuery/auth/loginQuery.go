...
const (
	...
	GetOrganizationQuery                 = "SELECT id, ifnull(name, ''), ifnull(phone, ''), ifnull(phone1, ''), ifnull(profession_id, ''), ifnull(known_as, ''), ifnull(case_types, ''), ifnull(staff_id, ''), ifnull(info, ''), ifnull(website, ''), ifnull(instagram, ''), sms_price, sms_credit, created_at FROM organization WHERE id = ?"
	GetSimpleOrganizationQuery           = "SELECT id, ifnull(name, '') FROM organization WHERE id = ?"
	GetOrganizationUsersQuery            = "SELECT user.id id, ifnull(user.fname, '') fname, ifnull(user.lname, '') lname, ifnull(user.tel, '') tel, ifnull(user.user_group_id, '') user_group_id, user.created created, user.last_login last_login, user.birth_date birth_date, user_group.name user_group_name, organization.name organization_name, organization.id organization_id, ifnull(user.file_id, '') file_id FROM (user LEFT JOIN user_group on user.user_group_id = user_group.id) LEFT JOIN organization on organization.id = user.organization_id WHERE user.organization_id = ? and user.user_group_id = ? LIMIT 10 OFFSET ?"
	...
	CreateAppointmentQuery               = "INSERT INTO `appointment`(`user_id`, `info`, `start_at`, `case_type`, `income`, `staff_id`, `is_vip`, `code`) VALUES (?,?,?,?,?,?,?)"
